...
(function(scope){
  ...
  gulp.task('p-scripts:package:now', function(){
    var jsBundles = [distDir + '/scripts/my-element.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/my-element.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/my-element.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/my-element.html')
